...
function TimerDisplay({ seconds, urgent }: { seconds: number; urgent: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-1.5 font-mono text-sm font-semibold tabular-nums px-3 py-1.5 rounded-lg transition-colors ${urgent ? 'bg-[#feeffd] text-[#d4a0d4] animate-pulse' : 'bg-slate-100 text-slate-700'}`}>
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto w-80 bg-slate-900 border-l border-slate-700 h-full overflow-y-auto p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-4 text-xs text-slate-400">
          ...
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-[#feeffd]0 inline-block" /> Wrong</span>
        ...
        <div className="space-y-2.5 mb-6">
          {currentQuestion.choices.map((choice) => {
            ...
            if (isRevealed) {
              ...
              else if (isSelected && !isCorrectChoice) style = 'border-red-500 bg-[#feeffd]0/10 text-red-300';
                  ...
                  <div className="space-y-1.5">
                    {unansweredList.map(({ q, i }) => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg bg-slate-800 hover:bg-slate-700 transition-all text-left group"
                      >
                        <span className="w-7 h-7 rounded bg-[#feeffd]0/20 border border-red-500/40 text-red-400 text-xs font-bold flex items-center justify-center shrink-0">{i + 1}</span>
                        <span className="flex-1 text-slate-300 text-xs line-clamp-2 group-hover:text-white transition-colors">{q.question.replace(/\*\*/g, '')}</span>
                        <ChevronRight className="w-3.5 h-3.5 text-slate-500 group-hover:text-slate-300 shrink-0" />
                      </button>
                    ))}
